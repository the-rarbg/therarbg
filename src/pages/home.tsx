      ...
      <div>
        ...
        <div>
            ...
            <div className='font-thin flex justify-evenly'>
              {
                data.map((item,index)=>{
                  return(
                    <div key={index} className='flex mx-2'><input className='mx-[6px] accent-primary checked:bg-primary' type="checkbox" /><label>{item} </label></div>
                  )
                })
              }
            </div>
          ...
          <div className='w-8/12 mx-auto  text-center grid grid-cols-4 gap-4'>
            ...
            {data.map((item, index) => {
              return (
                <div key={index} className="cursor-pointer w-[195px] h-[115px] pl-[26px] pr-[161px] pt-7 pb-16 bg-gray-200 bg-opacity-10 rounded-lg border-gray-200 border-opacity-30 flex-col justify-start items-start gap-3.5 inline-flex hover:bg-primary/10 hover:border-[1px] hover:border-primary/50">
